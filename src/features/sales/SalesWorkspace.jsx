...
function LeadList({ leads, onAddLead, onEditLead, onViewLead, onConvertLead }) {
  ...
  const convertLead = async lead => {
    ...
    try {
      ...
      // The parent operation displays the actionable error toast.
    } finally {
  ...
  const columns = useMemo(() => [
    ...
    { key: 'actions', label: 'Actions', required: true, render: lead => <span className="flex items-center gap-2" onClick={event => event.stopPropagation()}><button type="button" onClick={() => onEditLead(lead)} className="rounded-lg p-2 text-gray-500 hover:bg-gray-100" aria-label={`Edit ${lead.name}`}><Edit2 className="h-4 w-4" aria-hidden="true" /></button><button type="button" disabled={Boolean(convertingId)} onClick={() => void convertLead(lead)} className="rounded-lg p-2 text-indigo-600 hover:bg-indigo-50 disabled:cursor-wait disabled:opacity-50" aria-label={convertingId === lead.id ? `Creating deal from ${lead.name}` : `Create deal from ${lead.name}`} aria-busy={convertingId === lead.id}>{convertingId === lead.id ? <Clock className="h-4 w-4 animate-spin" aria-hidden="true" /> : <MoveRight className="h-4 w-4" aria-hidden="true" />}</button></span> },
  ], [convertingId, onEditLead]);
