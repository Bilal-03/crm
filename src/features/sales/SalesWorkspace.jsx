...
function attentionForDeal(deal) {
  if (!deal || deal.status !== 'open') return [];
  const attention = [];
  const today = todayKey();
  if (!deal.next_activity_date) attention.push({ key: 'no-next-activity', label: 'No next activity', tone: 'amber' });
  if (deal.next_activity_date && deal.next_activity_date < today) attention.push({ key: 'overdue', label: 'Overdue activity', tone: 'red' });
...
function SalesDashboard({ summary, deals, pipelines, activities, onNavigate, onOpenDeal, onAddLead }) {
  ...
  const attentionDeals = openDeals.map(deal => ({ deal, attention: attentionForDeal(deal) })).filter(item => item.attention.length).sort((a, b) => b.attention.length - a.attention.length || Number(b.deal.amount || 0) - Number(a.deal.amount || 0));
...
function DealList({ deals, pipelines, onCreate, onOpenDeal, onUpdateDeal }) {
  const [search, setSearch] = useState('');
  ...
  const columns = useMemo(() => [
    ...
    { key: 'next', label: 'Next activity', render: deal => deal.next_activity_date ? formatDate(deal.next_activity_date) : <span className="font-semibold text-amber-700">Not set</span> },
    { key: 'attention', label: 'Attention', render: deal => <div className="flex max-w-48 flex-wrap gap-1">{attentionForDeal(deal).slice(0, 2).map(item => <AttentionBadge key={item.key} item={item} />)}</div> },
  ], [stages]);
...
function DealCard({ deal, stages, onOpen, onMove }) {
  const stage = stageForDeal(deal, stages);
  const attention = attentionForDeal(deal);
...
function SalesPipeline({ deals, pipelines, onOpenDeal, onUpdateDeal, onAddDeal }) {
...
function DealDetailDrawer({ initialDeal, stages, activities, meetings, onClose, onEdit, onUpdateDeal, onNotify }) {
  ...
  const stage = stageForDeal(deal, stages);
  const relatedActivities = activities.filter(activity => activity.lead_id && activity.lead_id === deal.source_lead_id);
  const relatedMeetings = meetings.filter(meeting => meeting.lead_id && meeting.lead_id === deal.source_lead_id);
  const timeline = [...(deal.stage_history || []).map(item => ({ id: `stage-${item.id}`, type: 'stage', date: item.changed_at, title: item.from_stage_name ? `Moved from ${item.from_stage_name} to ${item.to_stage_name}` : `Entered ${item.to_stage_name}`, detail: `Changed by ${item.changed_by}` })), ...relatedActivities.map(item => ({ id: `activity-${item.id}`, type: 'activity', date: item.timestamp, title: item.message, detail: 'Activity' })), ...relatedMeetings.map(item => ({ id: `meeting-${item.id}`, type: 'meeting', date: item.date_time, title: item.title, detail: 'Meeting' }))].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
...
export default function SalesWorkspace({ page, loading = false, error, summary, leads, accounts, contacts, deals, pipelines, activities, meetings, onNavigate, onAddLead, onEditLead, onViewLead, onConvertLead, onCreateDeal, onUpdateDeal, onCreateAccount, onUpdateAccount, onCreateContact, onUpdateContact, onNotify }) {
  ...
  return <>
    ...
    {page === 'deals' && <DealList deals={deals} pipelines={pipelines} onCreate={openDealDialog} onOpenDeal={setSelectedDeal} onUpdateDeal={onUpdateDeal} />}
    {page === 'pipeline' && <SalesPipeline deals={deals} pipelines={pipelines} onOpenDeal={setSelectedDeal} onUpdateDeal={onUpdateDeal} onAddDeal={() => openDealDialog()} />}
